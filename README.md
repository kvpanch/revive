...
After extracting the archive, point `$LLVM_SYS_231_PREFIX` to it:
```sh
export LLVM_SYS_231_PREFIX=</path/to/the/extracted/archive>/target-llvm/gnu/target-final
```
...
The `Makefile` provides a shortcut target to obtain a compatible LLVM build, using the provided [revive-llvm](crates/llvm-builder/README.md) utility. Once installed, point `$LLVM_SYS_231_PREFIX` to the installation afterwards:

```sh
make install-llvm
export LLVM_SYS_231_PREFIX=${PWD}/target-llvm/gnu/target-final
```
...
To build the `resolc` Solidity frontend executable, make sure you have obtained a compatible LLVM build and did export the `LLVM_SYS_231_PREFIX` environment variable pointing to it (see [above](#LLVM)).
